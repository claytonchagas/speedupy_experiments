...
def finalPoseEstimation(
    ...
    if pose == "straight":
        ...
        slope_coordinate_wheel_straight = [
            x / y
            for x, y in zip(
                (y_right_coordinates - y_left_coordinates),
                (x_right_coordinates - x_left_coordinates),
            )
        ]

        arctan_Values_straight = abs(
            np.rad2deg(np.arctan(slope_coordinate_wheel_straight))
        ) + np.rad2deg(np.pi / 2)
        ...
        arctan_Values_left = abs(
            np.rad2deg(np.arctan(slope_coordinate_wheel_left))
        ) + np.rad2deg(np.pi / 2)
        ...
        for i in range(len(arctan_Values_right)):
            if arctan_Values_right[i] < 0:
                arctan_Values_right[i] = np.rad2deg(
                    np.arctan(slope_coordinate_wheel_right[i])
                ) + np.rad2deg(np.pi / 2)
...
def finalVisualization(
    x_left_wheel_right,
    y_left_wheel_right,
    x_left_wheel_left,
    y_left_wheel_left,
    x_left_wheel_straight,
    y_left_wheel_straight,
    x_right_wheel_right,
    y_right_wheel_right,
    x_right_wheel_left,
    y_right_wheel_left,
    x_right_wheel_straight,
    y_right_wheel_straight,
    x_center_right,
    y_center_right,
    x_center_left,
    y_center_left,
    ...
    X_center = np.hstack((x_center_right, x_center_left, x_center_straight))
    Y_center = np.hstack((y_center_right, y_center_left, y_center_straight))
...
def main(f_left, f_straight, f_right):
    # STRAIGHT MOTION
    X_left_wheel_straight = readData(f_straight)[0][36:].split(",")

    X_left_wheel_straight_new = typeConverter(X_left_wheel_straight, float)

    Y_left_wheel_straight = readData(f_straight)[1][36:].split(",")
    Y_left_wheel_straight_new = typeConverter(Y_left_wheel_straight, float)

    X_right_wheel_straight = readData(f_straight)[2][36:].split(",")
    X_right_wheel_straight_new = typeConverter(X_right_wheel_straight, float)

    Y_right_wheel_straight = readData(f_straight)[3][36:].split(",")
    Y_right_wheel_straight_new = typeConverter(Y_right_wheel_straight, float)

    x_final_pose_straight, y_final_pose_straight = finalPoseEstimation(
        "straight",
    ...
    # RIGHT MOTION
    X_left_wheel_right = readData(f_right)[0][33:].split(",")
    X_left_wheel_right_new = typeConverter(X_left_wheel_right, float)

    Y_left_wheel_right = readData(f_right)[1][33:].split(",")
    Y_left_wheel_right_new = typeConverter(Y_left_wheel_right, float)

    X_right_wheel_right = readData(f_right)[2][33:].split(",")
    X_right_wheel_right_new = typeConverter(X_right_wheel_right, float)

    Y_right_wheel_right = readData(f_right)[3][33:].split(",")
    Y_right_wheel_right_new = typeConverter(Y_right_wheel_right, float)

    x_final_pose_right, y_final_pose_right = finalPoseEstimation(
        "right",
        X_left_wheel_right_new,
...
if __name__ == "__main__":
    ...
    main(f1, f2, f3)  # left, straight, right
